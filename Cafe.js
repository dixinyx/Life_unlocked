...
class CafeGame {
  ...
  endShift() {
    ...
    this.state.customers.forEach(customer => {
      customer.patienceLastUpdatedAt = null;
    });
  ...
  updateCustomerPatience(activeWorkScreenOpen = true) {
    ...
    if (difficulty.patienceSeconds === null) {
      this.state.customers.forEach(customer => {
        customer.patienceRemainingMs = null;
  ...
  generateCustomer(source = "counter", options = {}) {
    ...
    const startingPatience = this.getCustomerPatienceMs();
    ...
    const customer = {
      ...
      arrivedAt: Date.now(),
      served: false,
      patienceRemainingMs: startingPatience,
      patienceLastUpdatedAt: CafeHelpers.now(),
      patienceWarningGiven: false
    };
  ...
  getFirstActionableCustomer(source = null) {
    const workArea = source || this.state.shift.workArea || "counter";

    return this.state.customers.find(customer => {
      if (customer.served || customer.source !== workArea) return false;
  ...
  getFirstActionableOrder(status, source = null) {
    const workArea = source || this.state.shift.workArea || "counter";

    return this.getActiveOrders().find(order =>
      order.status === status &&
      order.source === workArea
    ) || null;
  }

  takeNextOrderForWorkArea(source = null) {
    if (!this.state.shift.active) {
      return this.announce("Start a shift before taking an order.");
    }

    const customer = this.getFirstActionableCustomer(source);

    if (!customer) {
      return this.announce("There is no customer waiting to have an order taken.");
    }
  ...
  prepareWholeOrder(orderId) {
    const order = this.state.orders.find(item => item.id === orderId);

    if (!order) {
      this.announce("Order not found.");
      return null;
    }

    if (order.status === "ready") {
      this.announce(`${order.customerName}'s order is already ready to serve.`);
      return order;
    }

    const continuePreparation = () => {
      const current = this.state.orders.find(item => item.id === orderId);
      if (!current || current.status === "served" || current.status === "ready") return;

      if (current.status === "waiting") {
        this.prepareOrder(orderId);
      }

      window.setTimeout(continuePreparation, 250);
    };
  ...
  prepareNextOrderForWorkArea(source = null) {
    if (!this.state.shift.active) {
      return this.announce("Start a shift before preparing an order.");
    }

    const order = this.getFirstActionableOrder("waiting", source);

    if (!order) {
      return this.announce("There is no waiting order to prepare.");
    }

    this.prepareWholeOrder(order.id);
    return order;
  }

  serveNextOrderForWorkArea(source = null) {
    if (!this.state.shift.active) {
      return this.announce("Start a shift before serving an order.");
    }

    const order = this.getFirstActionableOrder("ready", source);

    if (!order) {
      return this.announce("There is no ready order to serve.");
    }
